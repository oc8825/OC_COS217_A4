...
/*
 ...
 to be the new node if successful. The Node object can be either a directory or a file, in which if it is a file the boolean isFile is set to TRUE, it has no children and its attributes relating to its contents and size are set accordingly. Otherwise, the opposite happens, its file status is set to FALSE, its children array is allocated in memory, and anything relating to its contents is set to FALSE. If it fails, then sets *poNResult to NULL
 and returns status:
...
/*
 ...
 oNNode, i.e., deletes this node and all its descendents only if it is a directory. If it is a file, just delete the file. Returns the
...
/*
 ...
 FALSE if it does not. Automatically returns FALSE if the Node is a file.
...
/* Returns the number of children that oNParent has, 0 automatically it is a file */
size_t Node_getNumChildren(Node_T oNParent);

/*
If the Node is a file, returns NOT_A_DIRECTORY, else behaves accordingly:
...
/*
Returns the contents of the ONNode, will be NULL if oNNode is not a file
*/
void *Node_getContents(Node_T oNNode);

/*
 Returns the size of the contents in ONNode, will be NULL if oNNode is not a file
*/
size_t Node_getContentsSize(Node_T oNNode);

/*
Returns the old contents of the Node and sets the new contents of the Node as well.
Will return NULL if oNNode is not a file.
*/
